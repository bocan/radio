...
const STATIONS = [
  {
    ...
    description:
      '<a href="https://somafm.com/" target=_blank>SomaFM</a> is a trailblazing commercial-free, listener-supported internet radio broadcaster based in San Francisco with a diverse catalog including ambient, downtempo, IDM, world beats, and more—often spotlighting indie and unsigned artists.',
    streams: [
      {
...

  {
    ...
    location: "Global",
    description:
      '<a href="https://nightride.fm/" target=_blank>Nightride FM</a> is an independent, community-driven radio station and digital platform specialising in synthwave and adjacent genres. It delivers 24/7 high-quality, ad-free streaming across multiple curated sub-stations.',
    streams: [
      {
...

  {
    ...
    description:
      "<a href='https://kutx.org/' target=_blank>KUTX</a> is deeply rooted in the Austin music scene. The on-air hosts handpick music with a collaborative editorial slant to highlight the city’s creative heartbeat—ranging from indie rock and hip-hop to electronica, outlaw country, jazz, blues, and psych-metal",
    streams: [
      {
...

  {
    ...
    description:
      "<a href='https://www.wwoz.org/' target=_blank>WWOZ</a> 99.7 FM is the New Orleans Jazz and Heritage Station, a community radio station currently operating out of the French Quarter in New Orleans. You can often catch this one playing some delta blues, jazz, or New Orleans funk.",
    streams: [
      {
...

  {
    ...
    description:
      "<a href='https://www.thelotradio.com/' target=_blank>The Lot Radio</a> is an independent, non-profit, online radio station live streaming 24/7 from a reclaimed shipping container on an empty lot in NYC. Expect a continuous stream of the best and most varied music New York City has to offer.",
    streams: [
      {
...

  {
    ...
    description:
      "<a href='https://www.radiofreenashville.org' target=_blank>Radio Free Nashville</a> is a true community radio station, completely locally owned and operated by volunteers from the Nashville and Middle Tennessee community. Local people create programming, design events, and keep the station up and running and on the air. ",
    streams: [
      {
        url: "https://ice23.securenetsystems.net/WRFNLP",
        type: "audio/aac",
      },
    ],
    metadataFormat: "xml", // 'xml' | 'json' (default json if omitted)
    metadataUrl:
      "https://streamdb8web.securenetsystems.net/player_status_update/WRFNLP.xml",
...

  {
    ...
    description:
      "<a href='https://kennetradio.com/' target=_blank>Kennet Radio</a> is my local community radio station, broadcasting on 106.7 FM to Newbury and Thatcham. Formed in October 2012 as a not-for-profit, volunteer-led organisation, they were awarded a Community Radio broadcasting licence by Ofcom in 2016.",
    streams: [
      {
        url: "https://stream.kennetradio.com/128.mp3",
        type: "audio/aac",
      },
    ],
    metadataFormat: "text",
...

  {
    ...
    parseNowPlaying: (data) => {
      ...
      const track_title = (track?.track_title || "")
        .trim()
        .replaceAll("&amp;", "&");
...

  {
    ...
    description:
      "<a href='https://ckut.ca' target=_blank>CTUK</a> is a non-profit, campus/community radio station based at McGill University in Montreal. CKUT provides alternative music, news and spoken word programming to the city of Montreal, surrounding areas, and around the world 24 hours a day, 365 days a year.",
    streams: [
      {
        url: "https://delray.ckut.ca:8001/903fm-192-stereo",
        type: "audio/mpeg",
      },
    ],
...
function updateNowPlaying(result, station) {
  ...
  if (typeof result === "string") {
    ...
    if ("artworkUrl" in result)
      artFromResult = (result.artworkUrl || "").trim();
  }
...
async function startStation(station) {
  ...
  const source =
    (station.streams || []).find(
      (s) => !s.type || els.audio.canPlayType(s.type)
    ) || station.streams?.[0];
  if (!source) {
  ...
  const isHls =
    /m3u8|application\/(x-)?mpegurl|application\/vnd\.apple\.mpegurl/i.test(
  ...
  if (isHls) {
    ...
    if (els.audio.canPlayType("application/vnd.apple.mpegurl")) {
      ...
      for (let i = 0; i < els.audio.textTracks.length; i++)
        onTrack(els.audio.textTracks[i]);
      // Newly-added tracks
      els.audio.textTracks.addEventListener?.("addtrack", (e) =>
        onTrack(e.track)
      );
      ...
      hls.on(Hls.Events.LEVEL_SWITCHED, (_e, d) =>
        console.log("LEVEL_SWITCHED ->", d.level)
      );
  ...
  els.audio
    .play()
    .catch((err) => console.warn("Autoplay blocked or play failed:", err));
  setPlayIcon(true);
    ...
    if (
      ...
      sse.onmessage = (ev) => {
        try {
          ...
          if (!item) return;
          if (item.station && item.station.toLowerCase() !== "nightride")
            return;
          const title = station.parseNowPlaying
            ? station.parseNowPlaying(item)
            : null;
      ...
      const poll = async () => {
        try {
          const url =
            typeof station.metadataUrlBuilder === "function"
          ...
          const wantsXml =
            station.metadataFormat === "xml" ||
            /(^|\/)(xml|html)\b/.test(ctype) ||
            (/text\/plain/.test(ctype) && station.metadataFormat === "xml");

          const wantsText =
            station.metadataFormat === "text" || /text\/plain/.test(ctype);

          const payload = wantsXml
            ? await res.text()
            : wantsText
            ? await res.text()
            : await res.json();

          const parsed = station.parseNowPlaying
            ? station.parseNowPlaying(payload)
            : null;
...
const defaultStation =
  STATIONS.find((s) => s.id === savedStationId) || STATIONS[0];
if (defaultStation) startStation(defaultStation);

// ...existing code...

// Add near the bottom with other init code
function initTicker() {
  ...
  function showMessage(i, withFade = true) {
    if (withFade) msgEl.classList.add("fade");
    setTimeout(() => {
      msgEl.innerHTML = TICKER_MESSAGES[i];
  ...
  function prev() {
    idx = (idx - 1 + TICKER_MESSAGES.length) % TICKER_MESSAGES.length;
    showMessage(idx);
    restartTimer();
  }
